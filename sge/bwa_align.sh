...
wd="/ifs/labs/cccb/projects/cccb/projects/2016/5/ZH_05172016_1085_combined"
sample_lanes="${wd}/lane_specific_fastq_sources.tsv"
...
for read1 in ${wd}/Sample_${f}/lane_specific_fastq/*R1*; do
    ...
    if [ ! -f $sam_file ] && [ ! -f $sort_file ]; then
        if [ $is_paired == "true" ]; then
            $bwa mem -R "$RG" $hg19 $read1 $read2 > $sam_file;
        else
            $bwa mem -R "$RG" $hg19 $read1 > $sam_file;
        fi;
    fi;
done;
