...
for f in `awk '{print $1}' $samples`; do
    ...
    echo "$java -jar $gatk -T Mutect2 -R $hg19 -I:tumor $bqsr_bam --genotyping_mode DISCOVERY -stand_emit_conf 10 -stand_call_conf 30 --dbsnp $dbsnp --cosmic $cosmic --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o $vcf" >> ${wd}/cmds.txt;
done;
